 ...
 *
 * Halaman modul Condition (kondisi & diagnosis).
 * Struktur konsisten dengan app/service-request/page.tsx (panel bertab).
 *
 * Alur:
 *   1. Panel SIMGOS bertab (5 jenis: Diagnosis, Anamnesis, Riwayat Penyakit
 *      Dahulu, Hasil PA, Penilaian Tumor) — read-only, highlight
 *      "Menunggu Encounter".
...
import ConditionForm from "@/app/components/modules/condition/ConditionForm";
...
import ConditionSyncPanel from "@/app/components/ihs/ConditionSyncPanel";
...
> = {
  POST: {
    title: "Catat Kondisi / Diagnosis",
    desc: "Kirim data Condition (terkait Encounter) ke Satu Sehat.",
    color: "teal",
    endpoint: "POST /Condition",
  },
  GET: {
    title: "Ambil Data Kondisi",
    desc: "Cari Condition berdasarkan ID, pasien, atau encounter.",
    color: "blue",
    endpoint: "GET /Condition",
  },
  PUT: {
    title: "Perbarui Kondisi (Penuh)",
    desc: "Timpa seluruh resource Condition yang sudah ada.",
    color: "amber",
    endpoint: "PUT /Condition/:id",
  },
  PATCH: {
    title: "Perbarui Kondisi (Sebagian)",
    desc: "Perbarui sebagian field Condition (mis. status klinis).",
    color: "violet",
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="w-11 h-11 rounded-2xl bg-linear-to-br from-blue-100 to-sky-100 border border-blue-200 flex items-center justify-center text-2xl shrink-0 shadow-sm">
              🩺
            </div>

            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-xl font-bold text-slate-900">Condition</h1>
                <span className="text-[10px] font-bold px-2 py-1 rounded-full bg-blue-100 text-blue-700">
                  FHIR R4
                </span>
              </div>
              <p className="text-sm text-slate-500 mt-0.5">
                Kondisi & diagnosis pasien — Satu Sehat Integration
              </p>
            </div>
          </div>
        ...
        <ConditionSyncPanel onUsePayload={handleUsePayload} />
        ...
        {/* ── 5. Log Pengiriman ── */}
        <DeliveryLogTable resourceType="Condition" />
      </div>
    </DashboardLayout>
  );
}

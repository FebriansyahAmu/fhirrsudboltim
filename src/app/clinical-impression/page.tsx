/**
 * app/clinical-impression/page.tsx
 *
 * Halaman modul ClinicalImpression.
 * Struktur dan pola konsisten dengan app/careplan/page.tsx.
 *
 * Terdiri dari:
 *   1. Page header (ikon, judul, env badge)
 *   2. Method tabs (GET, POST, PUT, PATCH)
 *   3. Info bar — deskripsi method yang aktif
 *   4. Request panel (form) + Response panel (viewer)
 *   5. Tabel log pengiriman
 */

"use client";

import { useState } from "react";

import DashboardLayout from "@/app/components/layout/DashboardLayout";
import ApiMethodTabs from "@/app/components/modules/ApiMethodTabs";
import ClinicalImpressionForm from "@/app/components/modules/clinical-impression/ClinicalImpressionForm";
import ResponseViewer from "@/app/components/ui/ResponseViewer";
import DeliveryLogTable from "@/app/components/ui/DeliveryLogTable";
import { useApiRequest } from "@/app/lib/hooks/useApiRequest";
import type { HttpMethod } from "@/app/lib/types/api";
import type { ClinicalImpressionPayload } from "@/app/lib/types/fhir";

// ─────────────────────────────────────────────
// Konstanta: deskripsi per method
// ─────────────────────────────────────────────

/**
 * Deskripsi dan warna aksen untuk setiap HTTP method.
 * Hanya method yang tersedia untuk modul ini yang perlu didefinisikan.
 */
const METHOD_INFO: Partial<
  Record<
    HttpMethod,
    { title: string; desc: string; color: string; endpoint: string }
  >
> = {
  POST: {
    title: "Buat ClinicalImpression Baru",
    desc: "Kirim penilaian klinis pasien baru ke Satu Sehat.",
    color: "teal",
    endpoint: "POST /ClinicalImpression",
  },
  GET: {
    title: "Ambil Data ClinicalImpression",
    desc: "Ambil penilaian klinis berdasarkan ID atau filter parameter.",
    color: "blue",
    endpoint: "GET /ClinicalImpression",
  },
  PUT: {
    title: "Perbarui ClinicalImpression (Penuh)",
    desc: "Timpa seluruh resource ClinicalImpression yang sudah ada.",
    color: "amber",
    endpoint: "PUT /ClinicalImpression/:id",
  },
  PATCH: {
    title: "Perbarui ClinicalImpression (Sebagian)",
    desc: "Perbarui sebagian field ClinicalImpression.",
    color: "violet",
    endpoint: "PATCH /ClinicalImpression/:id",
  },
};

/**
 * Mapping warna method ke kelas Tailwind untuk info bar.
 * Dipisahkan agar PurgeCSS tidak menghapus kelas yang dibangun secara dinamis.
 */
const INFO_BAR_COLOR: Record<string, string> = {
  teal: "bg-teal-50 border-teal-100 text-teal-800",
  blue: "bg-blue-50 border-blue-100 text-blue-800",
  amber: "bg-amber-50 border-amber-100 text-amber-800",
  violet: "bg-violet-50 border-violet-100 text-violet-800",
};

// ─────────────────────────────────────────────
// Page
// ─────────────────────────────────────────────

/** Method yang tersedia untuk modul ClinicalImpression */
const AVAILABLE_METHODS: HttpMethod[] = ["POST", "GET", "PUT", "PATCH"];

export default function ClinicalImpressionPage() {
  const [activeMethod, setActiveMethod] = useState<HttpMethod>("POST");

  // Hook mengelola state request/response dan fetch ke internal API Route
  const { apiResponse, sendRequest, resetResponse } = useApiRequest({
    resourceType: "ClinicalImpression",
  });

  /** Ganti method aktif dan bersihkan response sebelumnya */
  const handleMethodChange = (method: HttpMethod) => {
    setActiveMethod(method);
    resetResponse();
  };

  /** Diteruskan ke form — form yang memutuskan payload/resourceId/queryParams */
  const handleSubmit = async (params: {
    payload?: ClinicalImpressionPayload;
    resourceId?: string;
    queryParams?: Record<string, string | undefined>;
  }) => {
    await sendRequest({
      method: activeMethod,
      payload: params.payload,
      resourceId: params.resourceId,
      queryParams: params.queryParams,
    });
  };

  const info = METHOD_INFO[activeMethod];

  return (
    <DashboardLayout
      title="ClinicalImpression"
      breadcrumbs={[
        { label: "Dashboard", href: "/dashboard" },
        { label: "ClinicalImpression" },
      ]}
    >
      <div className="space-y-6">
        {/* ── 1. Page Header ── */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            {/* Ikon modul */}
            <div className="w-11 h-11 rounded-2xl bg-linear-to-br from-indigo-100 to-violet-100 border border-indigo-200 flex items-center justify-center text-2xl shrink-0 shadow-sm">
              🩺
            </div>

            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-xl font-bold text-slate-900">
                  ClinicalImpression
                </h1>
                <span className="text-[10px] font-bold px-2 py-1 rounded-full bg-indigo-100 text-indigo-700">
                  FHIR R4
                </span>
              </div>
              <p className="text-sm text-slate-500 mt-0.5">
                Penilaian klinis pasien — Satu Sehat Integration
              </p>
            </div>
          </div>

          {/* Environment badge */}
          <span className="shrink-0 flex items-center gap-1.5 text-xs text-emerald-700 bg-emerald-50 border border-emerald-200 px-3 py-1.5 rounded-xl font-medium w-fit">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
            Development
          </span>
        </div>

        {/* ── 2. Method Tabs ── */}
        <ApiMethodTabs
          methods={AVAILABLE_METHODS}
          activeMethod={activeMethod}
          onChange={handleMethodChange}
        />

        {/* ── 3. Info Bar ── */}
        {info && (
          <div
            className={`flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-4 py-3 rounded-2xl border ${
              INFO_BAR_COLOR[info.color]
            }`}
          >
            <div>
              <p className="text-sm font-semibold">{info.title}</p>
              <p className="text-xs opacity-70 mt-0.5">{info.desc}</p>
            </div>
            <code className="hidden sm:block text-[11px] font-mono bg-white/60 px-2.5 py-1.5 rounded-xl border border-current/10 whitespace-nowrap shrink-0">
              {info.endpoint}
            </code>
          </div>
        )}

        {/* ── 4. Request + Response Panel ── */}
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
          {/* Form panel */}
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 space-y-4 overflow-y-auto max-h-[72vh]">
            <div className="flex items-center gap-2">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">
                Request
              </span>
              <div className="h-px flex-1 bg-slate-100" />
            </div>

            <ClinicalImpressionForm
              method={activeMethod}
              loading={apiResponse.loading}
              onSubmit={handleSubmit}
            />
          </div>

          {/* Response panel */}
          <div className="h-[72vh]">
            <ResponseViewer response={apiResponse} />
          </div>
        </div>

        {/* ── 5. Log Pengiriman ── */}
        {/*
          DeliveryLogTable mem-filter log berdasarkan resourceType="ClinicalImpression".
          Log akan terisi otomatis setelah request dikirim via useApiRequest.
          Catatan: setelah DAL diimplementasikan, log ini akan datang dari DB,
          bukan dari state lokal.
        */}
        <DeliveryLogTable resourceType="ClinicalImpression" />
      </div>
    </DashboardLayout>
  );
}

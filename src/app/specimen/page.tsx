 ...
 *
 * Halaman modul Specimen (spesimen laboratorium).
 * Struktur & pola konsisten dengan app/allergy/page.tsx.
 *
 * Alur:
 *   1. Panel SIMGOS (read-only) — status kirim tabel `specimen`,
 *      highlight "Menunggu ServiceRequest" (spesimen butuh permintaan terkirim).
...
import SpecimenForm from "@/app/components/modules/specimen/SpecimenForm";
...
> = {
  POST: {
    title: "Catat Spesimen Baru",
    desc: "Kirim data spesimen lab (terkait ServiceRequest) ke Satu Sehat.",
    color: "teal",
    endpoint: "POST /Specimen",
  },
  GET: {
    title: "Ambil Data Spesimen",
    desc: "Cari spesimen berdasarkan ID, pasien, ServiceRequest, atau tanggal.",
    color: "blue",
    endpoint: "GET /Specimen",
  },
  PUT: {
    title: "Perbarui Spesimen (Penuh)",
    desc: "Timpa seluruh resource Specimen yang sudah ada.",
    color: "amber",
    endpoint: "PUT /Specimen/:id",
  },
  PATCH: {
    title: "Perbarui Spesimen (Sebagian)",
    desc: "Perbarui sebagian field Specimen (mis. status).",
    color: "violet",
    endpoint: "PATCH /Specimen/:id",
  },
};
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="w-11 h-11 rounded-2xl bg-linear-to-br from-teal-100 to-cyan-100 border border-teal-200 flex items-center justify-center text-2xl shrink-0 shadow-sm">
              🧪
            </div>

            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-xl font-bold text-slate-900">Specimen</h1>
                <span className="text-[10px] font-bold px-2 py-1 rounded-full bg-teal-100 text-teal-700">
                  FHIR R4
                </span>
              </div>
              <p className="text-sm text-slate-500 mt-0.5">
                Spesimen laboratorium — Satu Sehat Integration
              </p>
            </div>
          </div>
        ...
        {/* ── SIMGOS: status kirim (read-only) — highlight "Menunggu ServiceRequest" ── */}
        <ModuleSyncPanel
          module="specimen"
        ...
        {/* ── 5. Log Pengiriman ── */}
        <DeliveryLogTable resourceType="Specimen" />
      </div>
    </DashboardLayout>
  );
}

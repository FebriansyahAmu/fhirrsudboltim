...
// Satu resource CarePlan, dua jenis (tabel SIMGOS berbeda). Keduanya dikirim
...
const TABS: { key: string; label: string; icon: string; hint: string }[] = [
  {
    ...
    icon: "🗓️",
    hint: "Rencana perawatan berkelanjutan",
  },
  {
    key: "careplan-jadwal",
    label: "Rencana Kontrol",
    icon: "📆",
    hint: "Jadwal kontrol ulang pasien",
  },
];

export default function CarePlanSyncPanel({
  onUsePayload,
}: {
    ...
    <div className="space-y-3">
      <div>
        <p className="mb-2 flex items-center gap-1.5 text-xs font-semibold text-slate-500">
          ...
          Jenis CarePlan — pilih yang ingin dikirim
        </p>
        <div
          role="tablist"
          aria-label="Jenis CarePlan"
          ...
          {TABS.map((t) => {
            ...
            return (
              <button
                ...
                className={`inline-flex items-center gap-2 rounded-xl px-3.5 py-2 text-sm font-semibold transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-400/50 motion-reduce:transition-none ${
                  on

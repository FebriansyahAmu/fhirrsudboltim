...
import {
  ...
  type ProcedureGetValues,
} from "@/app/lib/schemas/procedure.schema";
...
const inputCls =
  "w-full bg-white border border-slate-200 rounded-xl px-3 py-2.5 text-sm text-slate-800 placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-violet-400/40 focus:border-violet-400 transition-all duration-150";
const inputErrCls =
...
function GetForm({
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<ProcedureGetValues>({
    resolver: yupResolver(
      procedureGetSchema,
    ) as unknown as Resolver<ProcedureGetValues>,
  });

  const onValid = (data: ProcedureGetValues) => {
    onSubmit({
      resourceId: data.procedureId || undefined,
      queryParams: {
      ...
      <Section title="Parameter Pencarian">
        ...
        >
          <input
            ...
            className={`${cx(!!errors.procedureId)} font-mono`}
            autoComplete="off"
          />
        </Field>
...
function MutationForm({
  ...
  method: HttpMethod;
  onSubmit: (params: { payload: ProcedurePayload; resourceId?: string }) => void;
  loading: boolean;
  autofillRaw?: AutofillRaw;
}) {
  ...
  const {
    ...
    resolver: yupResolver(
      procedureFormSchema,
    ) as unknown as Resolver<ProcedureFormValues>,
    defaultValues: {
      status: "completed",
      ...
      codeSystem: "icd-9-cm",
      codeValue: "87.44",
      codeDisplay: "Routine chest x-ray, so described",
      subjectPatientId: "",
      subjectDisplay: "",
      encounterId: "",
      performerId: "",
      performedStart: new Date().toISOString().slice(0, 16),
      performedEnd: "",
      note: "",
      procedureId: "",
    },
  ...
  const buildPayload = (data: ProcedureFormValues): ProcedurePayload => {
    const payload: ProcedurePayload = {
      ...
      code: {
        coding: [
          {
            system: CODE_SYSTEM_URI[data.codeSystem] ?? CODE_SYSTEM_URI["icd-9-cm"],
            code: data.codeValue,
      ...
      performedPeriod: {
        start: new Date(data.performedStart).toISOString(),
        ...(data.performedEnd
          ? { end: new Date(data.performedEnd).toISOString() }
          : {}),
      },
    };
    ...
    if (data.note && data.note.trim()) {
      payload.note = [{ text: data.note.trim() }];
    }

    return payload;
  };

  const onValidForm = (data: ProcedureFormValues) => {
    onSubmit({
      payload: buildPayload(data),
      resourceId: needsId ? data.procedureId || undefined : undefined,
    });
  };
  ...
  const handleRawSubmit = () => {
    ...
    setRawError(null);
    onSubmit({ payload: parsed as ProcedurePayload });
  };
        ...
        <form onSubmit={handleSubmit(onValidForm)} noValidate className="space-y-4">
          {needsId && (
            <Section title="Identifikasi">
              <Field label="ID Procedure" required error={errors.procedureId?.message}>
                <input
                  {...register("procedureId")}
                  type="text"
                  placeholder="xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx"
          ...
          <Section title="Informasi Dasar">
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field
                label="Waktu Mulai"
                required
                error={errors.performedStart?.message}
              >
                <input
                  {...register("performedStart")}
                  type="datetime-local"
                  className={cx(!!errors.performedStart)}
                />
              </Field>
            </div>
            <Field
              label="Waktu Selesai"
              hint="Opsional"
              error={errors.performedEnd?.message}
            >
              <input
                {...register("performedEnd")}
                type="datetime-local"
                className={cx(!!errors.performedEnd)}
              />
            </Field>
            <div className="grid grid-cols-2 gap-3">
          ...
          <Section title="Kode Tindakan (ICD-9-CM / SNOMED)">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <Field label="Nama" required error={errors.codeDisplay?.message}>
                <input
                  ...
                  type="text"
                  placeholder="Routine chest x-ray"
          ...
          <Section title="Catatan">
            <Field label="Catatan" hint="Opsional" error={errors.note?.message}>
              <textarea
                {...register("note")}
                rows={2}
                placeholder="Catatan tindakan..."
                className={`${cx(!!errors.note)} resize-none`}
              />
            </Field>
          </Section>
      ...
      {mode === "raw" && (
        <div className="space-y-2">
          ...
          <textarea
            ...
            rows={16}
            placeholder='{"resourceType": "Procedure", ...}'
          ...
          >
            {loading ? <LoadingSpinner /> : <>{method} /Procedure</>}
          </button>
        </div>
      )}
    </div>
...
function submitButtonCls(method: HttpMethod, loading: boolean) {
  ...
  const colors: Record<string, string> = {
    ...
    POST: "bg-violet-600 hover:bg-violet-700 text-white shadow-sm shadow-violet-200",
    PUT: "bg-amber-600 hover:bg-amber-700 text-white shadow-sm shadow-amber-200",
    PATCH:
      "bg-fuchsia-600 hover:bg-fuchsia-700 text-white shadow-sm shadow-fuchsia-200",
  };
